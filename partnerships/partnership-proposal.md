---
title: Partnership Proposal
search: false
head:
  - - meta
    - name: robots
      content: noindex, nofollow
---

# Partnership Proposal

**Partnership period:** 1 July 2026–30 June 2027

## Let’s Build Something Valuable Together

The Global AI Community connects **250,000+ members across 200+ chapters worldwide**. Our ecosystem includes a global chapter and event platform, Global AI Weekly with **70,000+ subscribers**, a YouTube channel with **50,000+ subscribers**, and events led by passionate local AI communities.

We would like to welcome the Partner into this ecosystem with a year-round package that combines global visibility, useful technical content, product experiences, and direct engagement with developers at six AgentCon 2027 events.

The proposal brings together:

1. A full Global AI Community Partner package for one year.
2. Sponsorship of six AgentCon 2027 events and recognition as a **Global AgentCon 2027 Sponsor**.

The standard combined value is **€24,000**. For this proposal, the annual €6,000 Partner package is included at no additional cost, bringing the total investment to **€18,000**.

## What We Want to Achieve

Together, we want to:

- Introduce the Partner’s products and services to relevant AI developers, data scientists, students, professionals, and community leaders.
- Give community members practical ways to experience those products through content, credits, licenses, tools, workshops, or events.
- Build meaningful brand awareness through trusted global channels and local communities.
- Position the Partner as an active supporter of accessible, community-led AI education.
- Create useful engagement while respecting the community’s independence and privacy.

## Proposal at a Glance

| Component | Included | Standard price | Proposed price |
| --- | --- | ---: | ---: |
| Global AI Community Partner | 12-month package | €6,000 | Included |
| Global AgentCon 2027 Sponsor | 6 events at €3,000 each | €18,000 | €18,000 |
| **Total** | **Annual partnership and 6 events** | **€24,000** | **€18,000** |

## 1. Annual Partner Package

The annual package gives the Partner a consistent presence across our content, channels, platform, and community programs.

| Partnership benefit | What is included | Quantity or timing |
| --- | --- | --- |
| **Brand recognition** | Partner name, logo, and website link on the Global AI Community partner page, plus use of the approved Partner designation | Full partnership period |
| **Global AI Weekly** | Educational or technical Partner articles, planned together and reviewed by our editorial team | 4 articles: one per quarter |
| **Additional articles** | Extra approved Global AI Weekly articles at a 50% discount on the standard €500 rate | €250 per article |
| **Technical video series** | A practical series about the Partner’s organization, technology, product, or service | Up to 4 episodes of no more than 30 minutes each |
| **In-person interview** | An additional interview when our schedules and locations align | Optional |
| **Global social media** | Posts supporting the partnership, content, technical series, product campaign, or another agreed initiative | 6 posts during the partnership period |
| **Product and services event campaign** | One coordinated campaign and event series where the Partner can offer credits, vouchers, licenses, subscriptions, cloud or API access, developer tools, training, or services. The campaign can use our event publishing, registration, attendee communication, and capacity management tools. | 1 campaign and event series, planned together |

We will work together on the topics, audience, schedule, channels, and format. Our goal is to make every activity useful to the community while giving the Partner a strong and relevant platform.

## 2. Global AgentCon 2027 Sponsorship

The Partner will sponsor six AgentCon 2027 events. AgentCon events are organized jointly by Global AI Community HQ and invited chapter leads and/or local organizers.

The Partner can share its preferred locations from the [AgentCon 2027 event list](/agentcon/events), including Bengaluru, Bangkok, Houston, Dallas, Toronto, New York, Silicon Valley, Ottawa, Melbourne, Sydney, Hong Kong, Tokyo, Mukono, Berlin, and Amsterdam. We will then build the final six-event portfolio together, based on availability, local readiness, venue capacity, timing, geographic balance, and the Partner’s priorities.

Each event targets **300 developers**, creating a combined target of **1,800 attendee places** across six events. This is an event-planning target; actual registrations and attendance will vary by location.

### Included Across Six Events

| Deliverable | Total included |
| --- | ---: |
| Partner speaking session of 25 or 45 minutes, or workshop of 60 to 90 minutes where the venue permits | 6: one per event |
| Sponsor listing on event pages | 6 |
| Partner content block in pre-event emails | 6 |
| Social media post announcing the sponsorship | 6 |
| Partner content block in post-event emails | 6 |
| Verbal sponsor recognition during opening remarks | 6 |
| Event report | 6 |
| Access to event photo collections | 6 |
| Banner or small stand placement, where venue rules and space permit | Up to 6 |

### Global Recognition

The six-event commitment includes:

- Recognition as a **Global AgentCon 2027 Sponsor**.
- Use of the Global AgentCon 2027 Sponsor designation.
- Logo and name recognition on the central AgentCon sponsor page.
- Visibility in relevant program-wide AgentCon communications and materials.
- Central coordination of the six-event portfolio by Global AI Community HQ.

## How We Will Work Together

We will start with a kickoff session to agree on the annual content calendar, preferred AgentCon locations, product campaign, technical series, and key contacts.

### Global AI Community Will

- Coordinate the annual Partner package and six AgentCon sponsorships.
- Work with the Partner on content topics, campaign ideas, event preferences, and schedules.
- Publish approved content and brand assets through the agreed community channels.
- Provide access to the agreed event-organizing capabilities.
- Keep activities useful, educational, and aligned with our community standards.
- Share links, event reports, photos, and an end-of-period delivery summary.

### The Partner Will

- Provide logos, brand assets, speakers, campaign details, and content by the agreed deadlines.
- Nominate a speaker or workshop facilitator for each sponsored AgentCon event.
- Provide a spokesperson and technical input for the video series.
- Supply physical banners or stands where these are used.
- Manage the availability, support, and redemption of its own product or service offers.
- Follow the Global AI Community Code of Conduct and branding guidelines.

## Investment

| Item | Calculation | Price |
| --- | ---: | ---: |
| Six AgentCon 2027 sponsorships | 6 × €3,000 | €18,000 |
| Annual Global AI Community Partner package | Standard price €6,000 | Included |
| **Total investment** |  | **€18,000** |

The invoice schedule and final delivery calendar will be agreed during onboarding.

## A Few Practical Notes

- AgentCon locations and dates are finalized together and confirmed in writing.
- Replacing a selected sponsored event is agreed together in writing.
- Event materials must be supplied at least four weeks before each AgentCon.
- Content and sessions are reviewed together to ensure they are useful and appropriate for the community.
- Community, subscriber, and attendee personal data is not shared with partners.
- Product visibility does not imply endorsement by the Global AI Community.
- The formal partnership agreement will capture the final schedule and standard sponsorship terms.

## Next Steps

1. Confirm the proposed package and **€18,000** investment.
2. Share preferred AgentCon 2027 locations.
3. Agree on the content calendar, technical series, and product campaign.
4. Complete the partnership agreement.
5. Hold a kickoff session and begin delivery.

To move forward, contact **[hq@globalai.community](mailto:hq@globalai.community)**.
